...
3. Execute o seguinte comando para criar o banco de dados (se ainda não existir):
...
*(Observação: a ferramenta `dotnet ef` precisa estar instalada. Se não estiver, instale-a com `dotnet tool install --global dotnet-ef` e lembre-se de adicionar ao PATH conforme as instruções do comando.)*
